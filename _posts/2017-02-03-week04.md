...
On Monday, we will discuss the homework problem on supply and demand for milk.  In this context, we will address the question of how to form prediction intervals in models that involve a transformation of the y variable, and we will also finish our discussion of R^2 and the decomposition of variance.

We will then work on a case study anchored in the first half of Chapter 4 of the course packet, about grouping variables in regression models.  The goal is to build a model to predict [flaws in a manufacturing process for printed circuit boards.](https://github.com/jgscott/learnR/blob/master/cases/solder/solder.md)
...
For Monday of next week, please read chapter 5, on quantifying uncertainty using the bootstrap.  We will cover the sections from "Confidence intervals and coverage" onward in class, so it is optional to have read that far by Monday.  You can skip the section at the very end on "Bootstrapped Prediction Intervals," which we will not cover in this course.    


### Videos  

For next week, please watch the following videos:  
